...
@test "XSS fails via http" {
  result=$(curl -s --location --globoff 'http://127.0.0.1:8080/exist/rest/db?_query=declare%20option%20exist%3Aserialize%20%22method%3Dhtml%20media-type%3Dtext%2Fhtml%22%3B%20element%20html%20{element%20script%20{%22alert(%27XSS%27)%22}}&_wrap=no' | grep -o 'XSS' | tr -d '\n')
  [ -z "$result" ]
}
