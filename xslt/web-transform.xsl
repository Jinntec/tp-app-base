...
>
    ...
    <xsl:template match="webapp:servlet[webapp:servlet-name = 'EXistServlet']">
        <xsl:copy>
            <xsl:apply-templates select="@*" />
            <xsl:apply-templates select="webapp:servlet-name" />
            <xsl:apply-templates select="webapp:servlet-class" />
            <xsl:apply-templates
                select="webapp:init-param[webapp:param-name != 'hidden' and webapp:param-name != 'xquery-submission' and webapp:param-name != 'xupdate-submission']" />

            <!-- Set REST server to hidden -->
            <init-param xmlns="http://xmlns.jcp.org/xml/ns/javaee">
